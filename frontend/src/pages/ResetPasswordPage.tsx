...
const ResetPasswordPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        setError('Forbidden access. Invalid or expired recovery link.');
      } else {
        setSession(session);
      }
    });
  ...
  const handlePasswordUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      // Wait for a few seconds and redirect to login
      ...
      >
        <div className="flex flex-col items-center gap-4 mb-10">
          ...
          <div className="text-center px-4">
            ...
            <p className="text-slate-400 text-sm mt-1">Strengthen your Documind brain credentials</p>
          </div>
        </div>
        ...
        {submitted ? (
          ...
          >
            ...
            <div className="space-y-2">
              <p className="text-white font-bold text-lg">Password Changed Successfully</p>
            ...
            >
                <div className="space-y-4">
                    <p className="text-red-400 font-bold text-lg">Invalid Access</p>
                    <p className="text-slate-400 text-sm">
                        Unauthorized or expired link used. Please request a new recovery link.
                    </p>
                </div>
                <Link 
                    to="/forgot-password"
